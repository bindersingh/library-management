...
class AddBooks extends Component
{
  constructor() {
   ...
   handleSubmit=()=>
   {
     ...
     if(isValid)
     {
      let ub=this.randomStringGenerator();
     this.addBooks(this.state,ub);
     this.setState({ubn:""});
     this.setState({title:""});
  ...
  async addBooks(state,ub)
   {
    ...
    try {
      await db.books.add(book);
       db.books.get({ubn:ub}).then ((bookDetails)=> {
      for(let test=1;test<state.quantity;test++)
      {
       ...
       quantity:1,publisher:state.publisher,author:state.author,parent_id:bookDetails.id});
       if(test==state.quantity-1)
       {
         this.setState({parent_book_id:bookDetails.id,parent_book_name:bookDetails.title});
       }
      }
    });
    }
     catch (e) {
        console.log("Duplicate Unique Book number (UBN)");
          {this.handleSubmit()}
        }
        this.setState({barcode:ub});
...
render()
{
          ...
          if(this.state.submit && this.state.parent_book_id && this.state.parent_book_name)
          {
            console.log(this.state.submit);
            console.log(this.state.parent_book_id);
            console.log(this.state.parent_book_name);
          return(<BooksBarcodePrint barcode={this.state.barcode}  bookId={this.state.parent_book_id} parent_book_name={this.state.parent_book_name}/>);
          }
